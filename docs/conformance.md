...
уровня отката). Одно следствие этого блока проверяется тестом уже сейчас и вынесено в строку B9:
раз в прод пойдёт `dual`, совместимость с 2025 доказывается на `dual`, а не только на эре, которую
к тому моменту выключат. На момент этого документа этап M7 не выполнялся; действующая процедура отката —
