...
| E10 | `SECURITY.md` формулирует угрозу как угон **state handle**, а не сессии, и называет, откуда на бессессионной ноге берётся идентичность вызывающего | план §1.2 E (M7.3), [`seps-1.md`](mcp-2026-07-28/seps-1.md) | `test/conformance/public-contract.test.ts` › `names every principal form callerPrincipal() can actually return`<br>`test/conformance/public-contract.test.ts` › `never calls a session hijackable without saying which era still has sessions` | покрыто — три формы принципала (`oauth:`, `bearer:`, `session:local-stdio`) вычитываются из тела `callerPrincipal()` в `src/core/pending-actions.ts`, а не перечисляются в тесте, поэтому четвёртая ветка или переименование stdio-фолбэка красят строку. Проверка слова «hijack» привязана к строке, которая его несёт, а не к окну вокруг: окно позволяло соседнему буллету поручиться за устаревшее предложение |
