...
Опишите задачу AI-клиенту обычными словами: через этот MCP-сервер он обращается к официальным API Avito. **148 инструментов по 18 API** работают с чатами, объявлениями, статистикой, продвижением и заказами. Сервер запускается на вашем компьютере или собственном сервере.

Вам нужны AI-клиент и аккаунт Avito с доступом к API. Модель работает в клиенте, а проект предоставляет подключение и инструменты. Для автоответов и отчётов по расписанию отдельно настройте агента или планировщик.

**[English version →](README.md)** · [Документация](docs/README.md) · [Подключение клиентов](docs/clients.ru.md) · [Сценарии](docs/workflows.ru.md) · [Решение проблем](docs/troubleshooting.ru.md)

> **Новое в v2.1.1:** пошаговая настройка, полные сценарии и диагностика на русском и английском, отдельные инструкции для демо и рабочего аккаунта. [Изменения версии](CHANGELOG.md).

| Что хотите сделать                   | Куда перейти                                                          |
| ------------------------------------ | --------------------------------------------------------------------- |
| Посмотреть пример без аккаунта Avito | [Запустить демо](#попробовать-без-доступов-avito)                     |
| Подключить свой аккаунт              | [Быстрый старт](#быстрый-старт)                                       |
| Настроить привычный AI-клиент        | [Конфигурации клиентов](docs/clients.ru.md)                           |
| Развернуть сервер или внести вклад   | [Эксплуатация](docs/operations.ru.md) · [Разработка](CONTRIBUTING.md) |
...
**В демо вымышленные данные: 53 объявления и 2 непрочитанных чата.** Оно работает локально, не требует ключей и не отправляет запросов в Avito. Вы увидите отчёт, предпросмотр цены, подтверждение и повтор запроса, при котором цена меняется только один раз. При первом запуске `npx` скачивает npm-пакет; из исходников — `npm run demo`.

**Для своего рабочего Avito выполните быстрый старт без `--demo`.** Обычные вызовы используют ваш настроенный аккаунт; демо не подменяет его данные.
...
| ------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------- |
| Проверить результаты объявлений | [Утренний отчёт](docs/workflows.ru.md#morning-report), [профиль analytics](examples/profiles/analytics.env.json)                             | Баланс, активность и расходы с датами и ID источников                        |
| Разобрать сообщения покупателей | [Непрочитанные чаты](docs/workflows.ru.md#unread-chat-triage), [профиль messenger](examples/profiles/messenger.env.json)                     | Приоритеты и черновики ответов; отправка — отдельное подтверждённое действие |
| Изменить цену объявления        | [Предпросмотр и подтверждение](docs/workflows.ru.md#preview-and-confirm-a-price-change), [профиль seller](examples/profiles/seller.env.json) | Точный запрос, ожидающее действие и проверка результата                      |
...
### 3. Проверьте чтение рабочих данных

Перезапустите клиент и спросите: **«Через Avito покажи мой баланс и первую страницу активных объявлений. Ничего не меняй».** В ответе должны быть ваш баланс и ID объявлений либо пустой список, если объявлений нет. Пример работает в `read_only`; без этой настройки сервер использует `full_access`.

**У VS Code, Zed, Codex и ChatGPT свои форматы настройки.** Скопируйте [конфигурацию своего клиента](docs/clients.ru.md), затем попробуйте [сценарий](docs/workflows.ru.md). Примеры фиксируют основную версию 2; для управляемого развёртывания укажите точную опубликованную версию.
...
Инструменты выдачи токенов и загрузки изображений скрыты до явного включения. Токены и рабочее состояние хранятся локально с ограниченными правами на файлы. Границы описаны в [настройках безопасности](docs/safety.md), [политике и приватном канале](SECURITY.md) и [диагностике](docs/troubleshooting.ru.md).
...
Вопросы и ошибки: [GitHub Issues](https://github.com/elchin92/avito-mcp/issues/new/choose) · [Поддержка](SUPPORT.md). Проекту помогут воспроизводимый баг, проверенная конфигурация клиента или более понятный сценарий. Если проект полезен вам, звезда на GitHub поможет другим пользователям Avito его найти.
