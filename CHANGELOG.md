...
Preparatory release before the MCP protocol migration, and a deliberate rehearsal of the release pipeline before the risky changes start. **Nothing moved on the wire: the server still speaks MCP revision `2025-11-25`, and this release adds no support for the `2026-07-28` revision.** Tool names, input schemas, resources, prompts, and configuration are unchanged — regenerating `dist/manifest.json` on the new SDK reproduces the 1.3.3 artifact byte for byte apart from its `version` field, and the manifest `schema_hash` is still `9c52d4c3f39300d267fba9bdcfb9a7aef9cb9664d325484f2a3967327f5f505f` across the same 148 tools. What did change is packaging discipline, dependency floors, and two test races. Upgrading from 1.3.3 is a drop-in. The release gate passes **388 tests across 34 files** with **81.43% statements / 73.57% branches / 82.06% functions / 84.37% lines** coverage.

### Packaging

- **The `files` allowlist no longer names `docs/` wholesale.** It now names `docs/safety.md` — the single file under `docs/` that backs a runtime resource (`avito://docs/safety`) — so a directory entry can no longer pack whatever happens to sit beside it. The concrete thing it was about to pack is the 2026-07-28 protocol corpus used for the migration: 66 entries, ~8.4 MB, kept untracked on purpose. `npm publish` runs from a clean checkout, so no published tarball ever contained it; but `npm pack` from a working tree that has the corpus produced **193 files / 2.8 MB compressed / 11.4 MB unpacked**, against **127 files / 481.2 kB / 2.7 MB** for the same tree without it. The allowlist, not the checkout, is now what decides that.
- **The image stops copying the whole `docs/` tree.** `COPY docs ./docs` read from the build context, so the corpus would have entered every image built on a machine that had it — the one place where the leak was real rather than hypothetical. The Dockerfile now copies `docs/safety.md` by name, and `.dockerignore` excludes the corpus and the migration notes as a second barrier.
- **Four barriers now hold this, and `test/release-hardening.test.ts` asserts all four.** The gate packs the real checkout and asserts no corpus path and no unlisted `docs/` path is present while `docs/safety.md` still is; re-packs the same `package.json` against a planted decoy tree, so the check keeps proving something in CI, where the corpus is never on disk; asserts the allowlist never names `docs/` as a directory again; and asserts the `.dockerignore` and `.gitignore` entries that keep the corpus out of the build context and out of git.

### Dependencies

- `@modelcontextprotocol/sdk` 1.29.0 → 1.30.0. The bump moves no tool schema: the manifest generated on 1.30.0 is identical to the published 1.3.3 manifest apart from the version field, `schema_hash` included.
- `express-rate-limit` 8.5.2 → 8.6.1.
- Development toolchain: `eslint` 10.6.0 → 10.8.0, `prettier` 3.9.5 → 3.9.6, `tsx` 4.23.0 → 4.23.1, `typescript-eslint` 8.63.0 → 8.65.0.
- CI actions, all pinned by commit SHA: `actions/checkout` 7.0.0 → 7.0.1, `actions/setup-node` 6.4.0 → 7.0.0. The `setup-node` pin annotation now reads `# v7`, matching the SHA it points at, so a reviewer comparing the comment to the pin is no longer misled.

### Testing

- **Two tests no longer race their own teardown**, the failure mode being a fire-and-forget write that lands after the suite removed the directory it writes into. The webhook durability test now awaits `WebhookStore.flush()` — the same drain the HTTP shutdown path uses — instead of sleeping 100 ms and hoping. The shared rate-limit test now awaits `RateLimiter.flushPersisted()` instead of polling for the file: polling proved only that the rename happened, while the lease was still open, and releasing it renames the lease to a `<file>.lock.transitioned-<id>` sibling that a concurrent `rm()` dies on with `ENOTEMPTY`.
- **`test/reliability-1.3.test.ts` moved off `os.tmpdir()` onto the repository filesystem** through the shared `test/support/sandbox.ts` helper, joining the other lease suites. Each store there publishes its lease with `mkdir` and must recognise that directory again; tmpfs never hands a freed inode back, which is exactly the filesystem where such an ownership check cannot fail.
- **A guard now rejects deprecated MCP surfaces before any of them can be introduced.** `test/deprecated-surface.test.ts` parses every `src/**/*.ts` with the TypeScript parser and fails on Sampling (`sampling/createMessage`, `createMessage()`), Roots (`roots/list`, `listRoots()`, `notifications/roots/list_changed`), `includeContext: "thisServer" | "allServers"`, and the HTTP+SSE transport (`SSEServerTransport`, `@modelcontextprotocol/sdk/*/sse.js`) — four features this server has never used, each already carrying a removal horizon in the `2026-07-28` deprecation registry. It scans code tokens only; comments are trivia and are not matched, so a deprecated name can still be written down where it explains an absence. `CONTRIBUTING.md` records the rule, the migration path for each row, and the two clarifications that make it usable: deprecated is not removed (Logging and Dynamic Client Registration stay), and an exception means editing the rule list with a stated reason, never deleting or skipping the test.
