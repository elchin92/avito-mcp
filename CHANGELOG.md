...
- **Prompt arguments are now validated, and a blank required one is refused instead of rendering a stub.** `prompts/get` is the one surface where a caller's string is copied verbatim into text a model reads as instructions — `avito_promote_item` names four tools and explains the confirmation flow that guards the money ones — and until now `tool_name` and `item_id` were unconstrained strings interpolated straight into that briefing. They are now allowlists (`^[a-z][a-z0-9_]{2,63}$` and 1–19 digits), `days` and `limit` are bounded decimal integers (1–365 and 1–100) instead of `parseInt(...) || default`, and every value is swept for control characters and bidi/zero-width formatting at the point it enters the text. Anything outside that is `-32602`, on **both** revisions. **This is a behaviour change on the 2025 wire**: `{ tool_name: "   " }` used to return a SUCCESSFUL result whose text asked the model to supply the value — an answer no client can tell apart from a prompt that rendered, so an agent would act on it. A missing required argument answered `-32602` before and still does; nothing about `prompts/list` moved.
