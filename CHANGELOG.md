...
- A timeout or dropped response after a mutation was sent now reports an unknown outcome and requires reconciliation before retry. Its idempotency reservation and confirmation claim remain held; read requests and failures before dispatch remain retryable. BBIP orders whose creation succeeded retain their order ID when later polling fails.
- The daily overview respects Avito's documented 270-day spending history and instructs the agent to count all listing pages or explicitly label an incomplete count. Legacy prompt argument parsing remains permissive, but its resulting date window is clamped; modern clients reject periods above 270 days.
- Updated vulnerable dependencies and pinned Zod at 4.4.3 to retain the existing schema representation for client compatibility.
- Corrected client-specific configuration, contributor guidance, community contacts and license metadata.

### Added

- `avito-mcp --demo` runs a real stdio MCP walkthrough against a local fictional Avito fixture: balance, paginated listings, unread chats, dry run, confirmation and idempotent replay. It needs no credentials and ignores the operator's live configuration. `--demo --json` emits its assertions as JSON.
- Copyable client configurations, focused analytics/messenger/seller profiles, three worked recipes, troubleshooting and a public roadmap.
- Full offline validation of the pinned MCP Registry schema and smoke checks for Windows and macOS.

### Compatibility

Tool names, input schemas, safety defaults and supported MCP revisions are unchanged. Error handling after an uncertain mutation and daily overview text are intentional corrections on both protocol eras. Clients must treat `OUTCOME_UNKNOWN` as a reconciliation request, not permission to retry with another key. The original 1.3.3 wire fixture remains immutable; the overview correction is separately accounted for by exact replacements.

Protocol negotiation remains unchanged: the default `legacy` mode serves MCP 2025-11-25 only. For protocol selection, nothing changes for an existing client until `AVITO_MCP_PROTOCOL_ERA` is set to `modern` or `dual` to enable MCP 2026-07-28. To roll back that protocol choice, restore `AVITO_MCP_PROTOCOL_ERA=legacy`; the reliability corrections above apply in every mode.
